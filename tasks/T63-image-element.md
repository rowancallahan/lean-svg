...
* `tests/svg/35_image.svg` (stored-block PNG, so any real decoder reads it)
...
| `run_tests.py` | 30/34 → 30/35; no existing file's score changed; the new `35_image` fails (51.7 %) until T61 lands |
| `run_adversarial.py` | 85/85 clean (77 before + 8 new: 4 checked-in, `image_data_30mb`, `image_bomb_uses`, `truncated_35_image`, `image_refs_inert`) |
