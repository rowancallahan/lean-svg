...
`fillMask (cv : Canvas) (m : Raster.Mask) (c : Rgba) (alpha8 : Nat)` with
`let a8 := Nat.min 255 alpha8`. T7's `(opacity256 : Nat)` parameter and its
`a8 := Nat.min 255 ((c.a * opacity256 + 128) >>> 8)` quantisation were dropped:
the opacity collapse now happens in `Svg.opacityToU8` before the call, and
`c.a` plays no part inside `fillMask`. Everything T7 added *inside* that
signature was taken unchanged: the `covNone = 128` / `covFull = 65408`
threshold constants with their derivation comment, the hoisted `solid` and
`fr fg fb fa`, the per-row `mrow` / `prow`, and the three-way per-pixel
...
* **Byte identity, 100/100.** Main's binary
  ...
  errors.** The two background sets matter: with a transparent canvas the
  full-coverage fast path writes over `dst = 0`, so only a filled background
  makes it reproduce a blend that actually depends on the destination.
* `python3 tests/run_tests.py`: **15/20 passed, 5 failed, 0 render errors**
  (tol=8, threshold=0.990). Run again with main's binary via `--bin` and the
  two `results.json` compared field by field: the only differences are the
  `bin` path, the `generated` timestamp and `ms_ours` / `ms_resvg`. Every
  fidelity metric — `exact%`, `within%`, `within32%`, `mean_abs`, `max_d`,
  per-test pass/fail — is identical to main's.
