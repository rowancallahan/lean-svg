# T95 — Markers: `paint-order` and `context-fill`/`context-stroke`  (branch `claude/fix-markers`)

Rowan reviewed the failure report and wants these fixed. resvg is rated
correct on all of them, so match resvg (the Rust source is the spec).

Files (`tests/corpora/resvg-test-suite/tests/`):
- `painting/paint-order/markers.svg`, `markers-stroke.svg`,
  `fill-markers-stroke.svg`: `paint-order` must order markers too (e.g. in
  `fill-markers-stroke` the orange corner marker is painted *under* the
  stroke). Today `LeanSvg/Marker.lean`'s `expand` always appends markers after
  fill and stroke, and only fill-vs-stroke order (`Style.strokeFirst`) exists.
- `painting/context/in-marker.svg`, `in-nested-marker.svg`,
  `in-nested-use-and-marker.svg`, `with-gradient-on-marker.svg`:
  `context-fill`/`context-stroke` inside marker content must resolve to the
  referencing shape's fill/stroke (including gradients). Today context paint
  is only wired for `<use>` content, so these markers vanish.

Rowan expects this to be an ordering/plumbing fix that needs no change to the
proofs; if a proof does need touching, fix it (never weaken it) and say why.
Also check `painting/marker/*` and `painting/context/*` stay green, and add
`tests/svg/95_marker_paint_order.svg`.
