...
* **Fidelity on the pattern-filled corpus files.** 21 of the corpus's 25
  ...
  `tests/svg/35_convolve.svg`'s four filters checked by eye against `resvg`).
...
* `tests/run_tests.py`: 32/36 passing, same 4 pre-existing unrelated fails as
  baseline, no file's score dropped; new `35_convolve` 99.998% within 8
...
* `tests/run_tiles.py`: 36/36 byte-identical, including `35_convolve`.
