...
- `python3 tests/run_tests.py`: 26/30 (the 4 failures are `main`'s
  ...
  99.530% within-8; three new files added, `41_text_decoration` 99.926%,
  `42_tref` 99.861%, `43_text_length` 99.920%.
...
parameter). `tests/svg/41_text_decoration.svg`, `tests/svg/42_tref.svg`,
`tests/svg/43_text_length.svg`.
hours; partial but regression-free beats complete but risky.
