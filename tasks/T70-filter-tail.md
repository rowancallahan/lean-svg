...
* `tests/run_tests.py`: every pre-existing file's score unchanged (same 4
  pre-existing failures, unrelated to filters); new `35_filter_tail`
...
* `tests/run_tiles.py`: 36/36 byte-identical, including the new
  `35_filter_tail` and its off-document/partial-tile transparency checks
  ...
  bug).
* Timing: the new adversarial file 37 ms; `35_filter_tail` typical local
