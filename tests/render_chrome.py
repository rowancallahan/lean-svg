...
Writes OUT_DIR/<parent-dir>__<basename>.chrome.png at WIDTH px wide, height from the SVG's
...
def main():
    ...
    for src, stem in pairs:
        # parent dir in the name: the suite reuses basenames across folders
        orig_stem = stem.split("_", 1)[1]
        dest = out_dir / ("%s__%s.chrome.png" % (Path(src).parent.name, orig_stem))
