...
renames a constructor. Catches a rename too: `.readInput`/`.writeOutput`
...
"""
    ...
    assert r.returncode == 0 and "error" not in r.stderr.lower(), (
        f"FAIL [two-effects]: Op is not exactly {{readInput, writeOutput}}:\n{r.stderr}"
    )
    print("-- two-effects: Op has exactly {readInput, writeOutput}")
