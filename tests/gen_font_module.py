...
static instance with `--instance`), moves the `glyf` table to the end of the
file (T94), then base64-encodes the result into `LeanSvg/Fonts/<Module>.lean`
as two `Array String`s of short chunks (rather than one giant string literal —
measured to keep `lake build` fast, see the T25 Report): `front`, every byte
before `glyf` (all the other tables, decoded whole when the font is first
used), and `glyf`, whose glyph records `Font.parseEmbedded` decodes one at a
time straight out of the chunks.  Plus `def coverage : String`, the subset's
...
def glyf_last(data: bytes) -> bytes:
    """The same font with its tables laid out in their current order except
    that `glyf` comes last, each 4-byte aligned, the directory (whose record
    order is unchanged) pointing at the new offsets.  Every table's bytes are
    unchanged, so the font parses to exactly the same glyphs and metrics."""
    dirs = tables(data)
    tags = [t for t, _, _, _ in dirs]
    assert tags.count(b"glyf") == 1, tags
    head_len = 12 + 16 * len(dirs)
    order = sorted(dirs, key=lambda d: (d[0] == b"glyf", d[2]))
...
def to_lean_module(data: bytes, module: str, chunk_chars: int, header: list[str]) -> str:
    ...
    data = glyf_last(data)
    glyf_off, glyf_len = [(o, n) for t, _, o, n in tables(data) if t == b"glyf"][0]
    front = chunked(data[:glyf_off], chunk_chars)
    glyf = chunked(data[glyf_off:], chunk_chars)
    lines = [
    ...
    lines += [
        ...
        "",
        "/-- Base64 of every font byte before the `glyf` table (the generator moves",
        "`glyf` to the end), split into short chunks so `lake build` never has to",
        "parse one giant string literal (see the T25 Report in",
        "tasks/T25-font-parser.md for the measured effect). -/",
    ...
    lines += [
        "]",
        "",
        f"/-- Base64 of the `glyf` table, {chunk_chars // 4 * 3} bytes per chunk; decoded",
        "one glyph record at a time (`Font.parseEmbedded`). -/",
        "def glyf : Array String := #[",
    ]
    lines += [f'  "{c}",' for c in glyf]
    lines += [
        "]",
        "",
        "/-- The font: `front` is decoded when it is first used, each glyph as it is drawn. -/",
        f"def font (_ : Unit) : Option LeanSvg.Font := LeanSvg.Font.parseEmbedded front glyf {chunk_chars // 4 * 3} {glyf_len}",
        "",
        "/-- The whole font file (for `fontdump`). -/",
        "def bytes (_ : Unit) : ByteArray := LeanSvg.Font.base64DecodeChunks (front ++ glyf)",
        "",
...
def module_bytes(path: Path) -> tuple[bytes, list[str]]:
    ...
    (either the T91 `chunks` form or this one's `front`/`glyf` form)."""
    src = path.read_text()
    header = [l[3:] for l in src.splitlines()[2:] if l.startswith("-- ")]
    arrays = re.findall(r"def (?:chunks|front|glyf) : Array String := #\[(.*?)\n\]", src, re.S)
    assert arrays, path
