...
**[rowancallahan.github.io/lean-svg](https://rowancallahan.github.io/lean-svg/)** —
what it supports, every test-suite file rendered next to resvg and your own
browser, the demos, and a drawing toy. Rebuilt from `main` on every push.
...
| what | licence |
|---|---|
| **resvg** and **usvg** — [linebender/resvg](https://github.com/linebender/resvg) | Apache-2.0 OR MIT — Copyright (c) 2017 Yevhenii Reizner. MPL-2.0 up to 0.44; [relicensed in 0.45.0](https://github.com/linebender/resvg/issues/838). The version read and used here is 0.48.1 |
...
transform parsing follow svgtypes' behaviour. No resvg, usvg, simplecss or
svgtypes source is included in this repository.
...
| what | licence |
|---|---|
| **resvg** binary, the rendering oracle | Apache-2.0 OR MIT (0.48.1) |
| **resvg test suite** (`tests/corpora/resvg-test-suite`) | Apache-2.0 OR MIT — it lives in the resvg repository and carries its licence |
...
`.gitignore`; none of them is committed here. The published site (`site/`,
built by `.github/workflows/pages.yml`) is the one place that redistributes
suite material: it republishes the suite's SVG files **unmodified** so a
visitor's browser can render the original, ships both of resvg's licence
texts, and credits the suite in every page footer and on its licensing page.
Nothing generated by that build is committed to this repository.
