...
def interpret (events : Array Xml.Event) : Except String Doc := do
  ...
  let applyEffective := fun (parent : Style) (attrs : Array Xml.Attr) (chain : Array Css.ElemInfo) =>
    ...
    let skipName (n : String) := n == "style" || n == "color" || n == "font-kerning"
      ...
      if skip > 0 then
        ...
        | none =>
          ...
          if isDisplayNone attrs || !passesConditions attrs then
            ...
            -- T36: seed the viewport that percentages resolve against, so
            -- `letter-spacing: 5%` and `x="29%"` on text have a reference.
            -- (`applyAttrs` did this; `applyEffective`, which replaced it,
            -- does not — see the Report.)
            let r := parseRoot attrs
            let (rw, rh) := match r.viewBox with
              | some (_, _, vw, vh) => (vw, vh)
              | none => (resolveRootSize r).getD (Fx.ofNat 100, Fx.ofNat 100)
            let rootParent : Style := { pctRefSet := true, pctRefW := rw, pctRefH := rh }
            stack := stack.push (applyEffective rootParent attrs chain)
            elemStack := chain
