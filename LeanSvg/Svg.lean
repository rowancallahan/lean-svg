...
def textShapes (applyEff : Style → Array Xml.Attr → Array Css.ElemInfo → Style)
  ...
  for p in placed do
    ...
    if st.visible then
      ...
      out := out.push ⟨p.cmds, { st with evenOdd := false, ctm := textStyle.ctm, crisp := false }, false⟩
  return (out, used)
...
def interpret (events : Array Xml.Event) : Except String Doc := do
  ...
  | some r => return ⟨r, nodes, clipsResolved, usesResolved, masksFixed, maskUsesFixed, markerTable, srcEvents⟩
